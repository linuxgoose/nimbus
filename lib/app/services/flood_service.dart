...
class FloodService {
  ...
  /// Parse SEPA station data
  static List<Map<String, dynamic>> _parseSEPAStations(
    dynamic data,
    double lat,
    double lon,
    double radiusKm,
    ...
    try {
      if (data is Map<String, dynamic>) {
        final items = data['stations'] as List<dynamic>?;

        if (items != null) {
          for (var item in items) {
            if (item is Map<String, dynamic>) {
              final sLat = item['latitude'] as double?;
              final sLon = item['longitude'] as double?;

              if (sLat != null && sLon != null) {
                final distance = _calculateDistance(lat, lon, sLat, sLon);
                if (distance <= radiusKm) {
                  stations.add({
                    'stationReference': item['station_id'] as String? ?? '',
                    'label': item['station_name'] as String? ?? 'SEPA Station',
                    'lat': sLat,
                    'long': sLon,
                    'notation': item['station_id'] as String? ?? '',
                    'riverName': item['river_name'] as String?,
                    'region': 'Scotland',
                  });
                }
              }
      ...
      debugPrint('✅ Parsed ${stations.length} SEPA stations');
    } catch (e) {
      debugPrint('❌ Error parsing SEPA stations: $e');
    }

    return stations;
  }

  /// Parse NRW station data
  static List<Map<String, dynamic>> _parseNRWStations(
    dynamic data,
    double lat,
    double lon,
    double radiusKm,
    ...
    try {
            ...
            if (item is Map<String, dynamic>) {
              ...
              if (sLat != null && sLon != null) {
                ...
                if (distance <= radiusKm) {
                  stations.add({
                    'stationReference': item['station_id'] as String? ?? '',
                    'label': item['station_name'] as String? ?? 'NRW Station',
                    'lat': sLat,
                    'long': sLon,
                    'notation': item['station_id'] as String? ?? '',
                    'riverName': item['river_name'] as String?,
      ...
      debugPrint('✅ Parsed ${stations.length} NRW stations');
    } catch (e) {
      debugPrint('❌ Error parsing NRW stations: $e');
    }
